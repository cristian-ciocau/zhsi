...
public class XPlaneWeatherReceiver extends StoppableThread {
	...
	public DatagramPacket receiveXPlanePacket() throws IOException {
		...
		/*
		 * Dont't notify XPlaneUDPSender, weather data can be send by a separate X-Plane
		 * instance
		 * 
		 * 
		 * if ( ! sender_known ) { // intercept the sender's (X-Plane's) address and
		 * port InetAddress orig_address = packet.getAddress(); int orig_port =
		 * packet.getPort();
		 * XPlaneUDPSender.get_instance().setDestination(datagram_socket, orig_address,
		 * orig_port); sender_known = true; }
		 */
		return packet;
	}
	...
	public void run() {
		...
		while (this.keep_running) {
			try {
				...
				ZHSIStatus.weather_receiving = true;
				...
				ZHSIStatus.weather_receiving = false;
				...
				logger.warning("Caught error while waiting for weather UDP packets! (" + e.toString() + " / "
						+ e.getMessage() + ")");
			}
		}
