...
public class NavigationObjectRepository {
	...
	// find the RadioNavigationObject, even at <RANGE_MULTIPLIER> times the
	// published range
	public static final float RANGE_MULTIPLIER = 1.25f;
	// let's hope that no other RadioNavigationObject with the same frequency is in
	// that multiplied range
	...
	public void add_nav_object(NavigationObject nav_object) {
		...
		if (nav_object instanceof RadioNavBeacon) {
			...
			if (get_airport(arpt_str) == null) {
				...
				add_arpt(nav_object);
				// } else {
				// logger.warning("NOT storing a duplicate Airport in the ArrayList for: " +
				// arpt_str);
			}
	...
	private void add_arpt(NavigationObject arpt_object) {
		...
		if (!this.airports.containsKey(arpt_str)) {
			...
			// } else {
			// logger.warning("NOT storing a duplicate Airport in the HashMap for: " +
			// arpt_str);
		}
	...
	// // TODO : a hashmap might be more efficient
	// ArrayList airport_list = get_nav_objects(NavigationObject.NO_TYPE_AIRPORT,
	// lat, lon);
	...
	public NavigationObject get_navobj(String ilt, float lat, float lon) {
		...
		// find the VOR, NDB, Waypoint or Airport with the supplied name that is close
		// to lat/lon
	...
	public Runway get_runway(String arpt, String rwy_id, float lat, float lon, boolean wide_search) {
		...
		while ((!found_rwy) && (index < rwy_list.size())) {
			...
			if (rwy.name.equalsIgnoreCase(arpt)
					&& (rwy.rwy_num1.equalsIgnoreCase(rwy_id) || rwy.rwy_num2.equalsIgnoreCase(rwy_id))) {
				found_rwy = true;
				// System.out.println(found_rwy);
			} else {
				index += 1;
			}
		}
			...
			for (int grid_lon = 0; grid_lon < 361; grid_lon++) {
				...
				while (index < navobj_list.size()) {
					...
					if (arpt.longest >= min_rwy) {
						...
						if (delta_lon > 270.0)
							delta_lon = 360.0 - delta_lon; // close to the Internationale Date Line, with one point on
						// either side
